...
BinaryOpWindow::BinaryOpWindow(MatrixLibrary* lib, QString name,
    QString logo, OpValidator v, QWidget* parent) : QDialog(parent)
{
    this->lib = lib;
    logoOp = logo;
    nameOp = name;
    ...
    QLabel* title = new QLabel(nameOp);
    ...
    connect(calculer, &QPushButton::pressed, this, &BinaryOpWindow::computeOperation);

    setLayout(mainLayout);
}


void BinaryOpWindow:: computeSelection (bool op)
{
    int selectedRow;
    ...
    if(!op)
    {
        ...
        formula->setText(selectedName + " " + logoOp + " " + op2Name);
    }
    else
    {
        ...
        formula->setText(op1Name + " " + logoOp + " " + selectedName);
    }
}


void BinaryOpWindow:: computeOperation ()
{
//    if(!validator(op1, op2))
//    {
