...
KerImgDimWindow::KerImgDimWindow(MatrixLibrary* lib, QWidget* parent) : QWidget (parent)
{
    ...
    QLabel* title = new QLabel("Dim(Ker) et Rang");
    ...
    connect(calculer, &QPushButton::clicked, this, &KerImgDimWindow::computeOperation);

    setLayout(mainLayout);
}


void KerImgDimWindow:: computeSelection ()
{
    int selectedRow;
...
void KerImgDimWindow:: computeOperation ()
{
    if(op1 == nullptr)
    {
    ...
    result = op1->dimensionsStudy();
    resultImg->computeImgDimMatrix(result, op1Name->text(), QColor(0,0,0,0));
    resultImg->show();
}


KerImgDimWindow:: ~KerImgDimWindow ()
{
}
